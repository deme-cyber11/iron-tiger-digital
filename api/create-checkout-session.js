...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (priceType === 'bundle') {
      // Bundle: Both setup (one-time) and monthly (recurring) in same checkout
      sessionConfig = {
        payment_method_types: ['card'],
        line_items: [
          { price: prices.setup, quantity: 1 },   // One-time setup fee
          { price: prices.monthly, quantity: 1 }  // Recurring monthly
        ],
        mode: 'subscription',
